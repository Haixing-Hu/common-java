...
public class RandomUserAgent {
  ...
  public static final String[] ALL_BROWSERS = {
      INTERNET_EXPLORER,
      FIREFOX,
      CHROME,
      SAFARI,
  ...
  private static final Map<String, String[]> USER_AGENT_MAP = new HashMap<>();

  private static final Map<String, Double> FREQUENCY_MAP = new HashMap<>();

  static {

    // stop checkstyle: MagicNumber
    FREQUENCY_MAP.put(INTERNET_EXPLORER, 11.8D);
    FREQUENCY_MAP.put(FIREFOX, 28.2D);
    FREQUENCY_MAP.put(CHROME, 52.9D);
    FREQUENCY_MAP.put(SAFARI, 3.9D);
    FREQUENCY_MAP.put(OPERA, 1.8D);
    // resume checkstyle: MagicNumber

    // stop checkstyle: LineLength
    USER_AGENT_MAP.put(INTERNET_EXPLORER,
        new String[] {
    ...
    USER_AGENT_MAP.put(OPERA,
        new String[] {"Opera/9.80 (Windows NT 6.0) Presto/2.12.388 Version/12.14",
            ...
            "Opera/9.99 (X11; U; sk)", "Opera/9.99 (Windows NT 5.1; U; pl) Presto/9.9.9",
  ...
  public static String randomChooseBrowser(final String ... browsers) {
    if (browsers.length == 0) {
      throw new IllegalArgumentException("Must specify at least on browser.");
    }
    final double sumFreq = Arrays.stream(browsers)
        .map(FREQUENCY_MAP::get)
        .reduce(0.0, Double::sum);
    final double rand = Math.random() * sumFreq;
    double count = 0.0;
    for (final String browser : browsers) {
      count += FREQUENCY_MAP.get(browser);
      if (rand <= count) {
        return browser;
      }
    }
    final RandomEx random = new RandomEx();
    return random.choose(browsers);
  }
}
