...
public class NamedThreadLocal<T> extends ThreadLocal<T> {
   ...
   *
   * @param <S>
   *     the type of the named thread local's value
   * @param name
   *     a descriptive name for the thread local
   * @param supplier
   *     the supplier to be used to determine the initial value
   * @return a new named thread local
   * @since 6.1
   */
  public static <S> ThreadLocal<S> withInitial(final String name,
      final Supplier<? extends S> supplier) {
    return new SuppliedNamedThreadLocal<>(name, supplier);
  }

  /**
   * An extension of NamedThreadLocal that obtains its initial value from the
