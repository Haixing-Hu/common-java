...
public interface ShortList extends ShortCollection {
  ...
  /**
   * Returns {@code true} iff <i>that</i> is an {@code ShortList} that contains
   * the same elements in the same order as me.
   *
   * <p>In other words, returns {@code true} iff <i>that</i> is a {@code
   * ShortList} that has the same {@link #size() size} as me, and for which the
   * elements returned by its {@link ShortList#iterator iterator} are equal
   ...
   * of the {@code ShortList} interface.)
   *
   * @param that
   *     the object to compare to me.
   * @return {@code true} iff <i>that</i> is an {@code ShortList} that contains
   *     the same elements in the same order as me.
   */
  @Override
   ...
   *
   * <p>The hash code of an {@code ShortList} is defined to be the result of
   ...
   * #equals equals} and with the {@link java.util.List#hashCode hashCode}
  ...
  /**
   * Returns an {@link ShortIterator iterator} over all elements, in the
   * appropriate sequence.
   *
   * @return an {@link ShortIterator iterator} over all elements.
   */
  @Override
   ...
   *
   * <p>The specified <i>current</i> indicates the first element that would be
   ...
   * @param index
   *     the specified index.
   * @return the specified {@link ShortListIterator iterator}.
   * @throws IndexOutOfBoundsException
   *     if the specified index is out of range.
   */
   ...
   *
   * <p>Any subsequent elements are shifted to the left, subtracting one from
   * their ndices. Returns the element that was removed.
   *
   * @param index
   ...
   * <i>fromIndex</i>, inclusive, and <i>toIndex</i>, exclusive.
   *
   * <p>The returned {@code ShortList} is backed by me, so that any changes in
   * the returned list are reflected in me, and vice-versa. The returned list
   ...
   * &amp;&amp;<i>toIndex</i> == {@link #size() size()}</code> the returned list
   * is my "improper" sublist, containing all my elements.
   *
