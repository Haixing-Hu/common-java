...
public interface LongList extends LongCollection {
   ...
   *
   * <p>将当前在该位置的元素（如果有）和任何后续元素向右移动，增加它们的索引。
   * 新元素将按照给定集合的 {@link LongCollection#iterator iterator} 返回的顺序出现。
   *
   * @param index
   *     要插入指定集合中第一个元素的索引。
   * @param collection
   *     要添加的元素的 {@link LongCollection LongCollection}。
   * @return 当且仅当我因此调用而更改时，才返回 {@code true}。
   * @throws UnsupportedOperationException
   *     当不支持此操作时。
   * @throws IndexOutOfBoundsException
  ...
  /**
   * 当且仅当 <i>that</i> 是一个 {@code LongList}，其中包含与我相同顺序的相同元素时，
   * 才返回 {@code true}。
   *
   * <p>换句话说，当且仅当 <i>that</i> 是一个 {@code LongList}，其 {@link #size() size} 与我相同，
   * 并且其 {@link #iterator iterator} 返回的元素等于（{@code ==}）我内部的相应元素时，
   * 才返回 {@code true}。（此约定确保此方法在 {@code LongList} 接口的不同实现中正常工作。）
   *
   * @param that
   *     要与我比较的对象。
   * @return 当且仅当 <i>that</i> 是一个 {@code LongList}，其中包含与我相同顺序的相同元素时，
   *     才返回 {@code true}。
   */
  @Override
   ...
   * 返回我的哈希码。
   *
   * <p>一个 {@code LongList} 的哈希码定义为以下计算的结果：
   *
   * <pre>
   * int hash = 1;
   ...
   * </pre>
   *
   * <p>此约定确保此方法与 {@link #equals} 以及
   * {@link java.util.List} of {@link Long}s 的
  ...
  /**
   * 返回一个覆盖所有元素的 {@link LongIterator iterator}，按适当的顺序。
   *
   * @return 一个覆盖所有元素的 {@link LongIterator iterator}。
   */
  @Override
  ...
  /**
   * 返回一个覆盖我所有元素的 {@link LongListIterator bidirectional iterator}，按适当的顺序。
   *
   * @return 一个覆盖所有元素的 {@link LongListIterator iterator}。
   */
  LongListIterator listIterator();

  /**
   * 返回一个覆盖我所有元素的 {@link LongListIterator bidirectional iterator}，
   * 按适当的顺序，从指定位置开始。
   *
   * <p>指定的<i>当前</i>位置表示初始调用 {@link LongListIterator#next next} 方法将返回的第一个元素。
   * 初始调用 {@link LongListIterator#previous previous} 方法将返回具有指定<i>当前</i>位置减一的元素。
   *
   * @param index
   *     指定的索引。
   * @return 指定的 {@link LongListIterator iterator}。
   * @throws IndexOutOfBoundsException
   *     如果指定的索引超出范围。
   */
   ...
   *
   * <p>返回的 {@code LongList} 由我支持，因此返回列表中的任何更改都会反映在我身上，反之亦然。
   * 返回的列表支持我支持的所有可选操作。
   *
