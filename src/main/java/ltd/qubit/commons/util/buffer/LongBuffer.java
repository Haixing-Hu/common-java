 ...
 * @author Haixing Hu
 */
public final class LongBuffer implements Swappable<LongBuffer>,
    Assignable<LongBuffer>, CloneableEx<LongBuffer>, Comparable<LongBuffer>,
    Serializable {

  private static final long serialVersionUID = 1883800278534004356L;

  static {
    register(LongBuffer.class, LongBufferBinarySerializer.INSTANCE);
  }
  ...
  public long bytes() {
    return Size.REFERENCE + Size.INT + (buffer.length * Size.LONG);
  }
   ...
   *         occur.
   */
  public int indexOf(final long value, final int beginIndex, final int endIndex) {
   ...
   *         occur.
   */
  public int lastIndexOf(final long value, final int beginIndex, final int endIndex) {
    final int begin = Math.max(0, beginIndex);
    final int end = Math.min(length, endIndex);
    if (begin > end) {
      return - 1;
