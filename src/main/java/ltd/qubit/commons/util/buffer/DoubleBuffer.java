 ...
 * @author Haixing Hu
 */
public final class DoubleBuffer implements Swappable<DoubleBuffer>,
    Assignable<DoubleBuffer>, CloneableEx<DoubleBuffer>,
    Comparable<DoubleBuffer>, Serializable {

  private static final long serialVersionUID = - 4277339416181320082L;

  static {
    register(DoubleBuffer.class, DoubleBufferBinarySerializer.INSTANCE);
  }
  ...
  public long bytes() {
    return Size.REFERENCE + Size.INT + (buffer.length * Size.DOUBLE);
  }
   ...
   */
  public int indexOf(final double value, final int beginIndex, final int endIndex,
   ...
   *     within the given bounds, or {@code -1} if the value does not occur.
   */
  public int lastIndexOf(final double value, final int beginIndex, final int endIndex,
      final double epsilon) {
    final int begin = Math.max(0, beginIndex);
    final int end = Math.min(length, endIndex);
    if (begin > end) {
